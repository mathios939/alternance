...
export function CompanyLogo({ name, logoUrl, size = "md", className }: { name: string; logoUrl?: string | null; size?: "sm" | "md" | "lg"; className?: string }) {
  const sizes = { sm: "size-8 text-[10px]", md: "size-11 text-xs", lg: "size-16 text-base rounded-2xl" };
  if (logoUrl) {
    // eslint-disable-next-line @next/next/no-img-element
    return <img src={logoUrl} alt="" className={cn("shrink-0 rounded-xl border bg-card object-contain p-1", sizes[size], className)} />;
  }
  return (
    <span aria-hidden className={cn("flex shrink-0 items-center justify-center rounded-xl border bg-gradient-to-br from-muted to-secondary font-semibold text-muted-foreground", sizes[size], className)}>
      {initials(name)}
    </span>
  );
}
...
export function JobCard({ job, variant = "default", selected = false, onSelect, isAuthenticated, className }: Props) {
  ...
  function onApply(e: React.MouseEvent) {
    ...
    startTransition(async () => {
      ...
      if (!result.ok) toast.error(result.error);
      else toast.success(result.data.created ? "Ajoutée à tes candidatures (À candidater)" : "Déjà dans tes candidatures", { action: { label: "Voir", onClick: () => router.push("/applications") } });
      router.refresh();
    });
  }
  ...
  const content = (
    <>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
        ...
        {!isCompact ? <span className="inline-flex items-center gap-1"><Building2 className="size-3.5" aria-hidden /> {CONTRACT_TYPES[job.contractType].short}</span> : null}
      </div>
