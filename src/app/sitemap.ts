...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const now = new Date();
  const staticPages: MetadataRoute.Sitemap = ["", "/jobs", "/companies", "/radar", "/map", "/sources", "/tarifs", "/confidentialite", "/cgu", "/mentions-legales"].map((p) => ({ url: `${base}${p}`, lastModified: now, changeFrequency: p === "" || p === "/jobs" ? "daily" : "monthly", priority: p === "" ? 1 : 0.6 }));
  ...
  try {
    const [companyRows, jobRows] = await Promise.all([
      prisma.company.findMany({ where: { isPlaceholder: false, ...demoFilter() }, select: { slug: true, updatedAt: true }, take: 5000 }),
      prisma.job.findMany({ where: { isActive: true, canonicalJobId: null, verificationStatus: { notIn: ["EXPIRED", "REMOVED"] }, ...demoFilter() }, select: { slug: true, updatedAt: true }, take: 20000 }),
    ]);
