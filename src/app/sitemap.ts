...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const now = new Date();
  const staticPages: MetadataRoute.Sitemap = ["", "/jobs", "/companies", "/sources", "/tarifs", "/confidentialite", "/cgu", "/mentions-legales"].map((p) => ({ url: `${base}${p}`, lastModified: now, changeFrequency: p === "" || p === "/jobs" ? "daily" : "monthly", priority: p === "" ? 1 : 0.6 }));
