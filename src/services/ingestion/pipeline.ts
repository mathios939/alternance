...
import { calculateDuplicateConfidence, type DedupeCandidate, type DuplicateVerdict } from "@/services/job-sources/dedupe";
import { normalizeJob, UNKNOWN_EMPLOYER_NAME } from "@/services/job-sources/normalize";
import type { FetchParams, IngestReport, JobSourceProvider, NormalizedJob, ProviderStatus } from "@/services/job-sources/types";
import { findOrCreateCompanyForJob } from "./company-match";
...
class DedupeIndex {
  ...
  add(c: DedupeCandidate) {
    ...
    if (company && company !== normalizeText(UNKNOWN_EMPLOYER_NAME)) this.byCompany.set(company, [...(this.byCompany.get(company) ?? []), c]);
    this.size++;
  }
  ...
  find(candidate: DedupeCandidate): { match: DedupeCandidate; verdict: DuplicateVerdict } | null {
    ...
    for (const c of this.byCompany.get(normalizeText(candidate.companyName)) ?? []) pool.set(c.id, c);
...
function toCandidate(job: NormalizedJob, id = "new"): DedupeCandidate {
  return { id, title: job.title, normalizedTitle: job.normalizedTitle, companyName: job.companyName, city: job.city, postalCode: job.postalCode, description: job.description, sourceUrl: job.sourceUrl, applicationUrl: job.applicationUrl, publishedAt: job.publishedAt, sourceKey: job.sourceKey, externalId: job.externalId };
}
...
async function upsertSource(provider: JobSourceProvider, status: ProviderStatus) {
  return prisma.jobSource.upsert({
    ...
    update: { name: provider.name, type: provider.type, priority: provider.priority, capabilities: provider.capabilities as unknown as Prisma.InputJsonValue },
    create: { key: provider.key, name: provider.name, type: provider.type, isEnabled: status.configured, priority: provider.priority, capabilities: provider.capabilities as unknown as Prisma.InputJsonValue },
  });
}
...
function entryColumns(job: NormalizedJob, now: Date) {
  return {
    ...
    applicationEmail: job.applicationEmail,
    applicationLabel: job.applicationLabel,
    rawPayload: job.raw === null || job.raw === undefined ? Prisma.JsonNull : (job.raw as Prisma.InputJsonValue),
    publishedAt: job.publishedAt,
    expiresAt: job.expiresAt,
...
export async function refreshCanonicalApplication(jobId: string): Promise<void> {
  const entries = await prisma.jobSourceEntry.findMany({ where: { jobId, status: { in: ["ACTIVE", "UNKNOWN"] } }, include: { source: { select: { priority: true } } }, orderBy: { firstSeenAt: "asc" } });
  ...
  await prisma.$transaction([
    ...
    prisma.job.update({ where: { id: jobId }, data: { applicationUrl: best.applicationUrl ?? best.url ?? undefined, applicationEmail: best.applicationEmail ?? undefined, applicationLabel: best.applicationLabel ?? undefined } }),
  ]);
}
...
export async function runIngestion(options: RunIngestionOptions): Promise<IngestReport> {
  ...
  const startedAt = now();
  const report: IngestReport = { sourceKey: provider.key, runId: null, fetched: 0, created: 0, updated: 0, duplicates: 0, rejected: 0, failed: 0, errors: [], warnings: [], durationMs: 0 };

  const status = await provider.status();
  const source = await upsertSource(provider, status);
  if (!status.configured) {
    const reason = status.reason ?? "Provider non configuré";
    const run = await startRun({ sourceKey: provider.key, sourceId: source.id, trigger, params, status: "SKIPPED", errorSummary: reason });
    await prisma.jobSource.update({ where: { id: source.id }, data: { lastSyncStatus: "DISABLED", lastSyncError: reason } });
    report.runId = run.id;
    report.errors.push(reason);
    report.durationMs = now().getTime() - startedAt.getTime();
    log.warn("Ingestion ignorée : provider non configuré", { provider: provider.key, operation: "ingest", status: "skipped", missing: status.missing });
    return report;
  }
  ...
  log.info("Ingestion démarrée", { provider: provider.key, operation: "ingest", runId: run.id, trigger, params });
  ...
  try {
    ...
    log.info("Récupération terminée", { provider: provider.key, operation: "fetch", durationMs: Date.now() - fetchStarted, status: "ok", count: page.jobs.length, total: page.total, requests: page.requests });

    const jobs = options.maxJobs ? page.jobs.slice(0, options.maxJobs) : page.jobs;
    const index = await loadDedupeIndex(now());
    ...
    for (const raw of jobs) {
      try {
        ...
        // DEDUPLICATION 1 : même source, même identifiant → mise à jour
        const existingEntry = await prisma.jobSourceEntry.findUnique({ where: { sourceId_externalId: { sourceId: source.id, externalId: job.externalId } }, select: { id: true, jobId: true } });
        if (existingEntry) {
          await prisma.$transaction([
            prisma.jobSourceEntry.update({ where: { id: existingEntry.id }, data: entryColumns(job, ts) }),
            prisma.job.update({ where: { id: existingEntry.jobId }, data: { ...jobColumns(job, ts), companyNameRaw: job.companyNameRaw } }),
          ]);
          report.updated++;
          continue;
        }
        ...
        // ENRICHMENT : entreprise, compétences, qualité, contact publié
        const company = await findOrCreateCompanyForJob(job, { sourceKey: provider.key, sourceLabel: provider.name, now: ts });
        if (company.created) companiesCreated++;
        const skillIds = job.skillSlugs.map((slug) => skillIdBySlug.get(slug)).filter((id): id is string => Boolean(id));
        ...
        const created = await prisma.job.create({
          data: {
            ...
            sourceEntries: { create: { sourceId: source.id, externalId: job.externalId, isPrimary: true, ...entryColumns(job, ts) } },
          },
          select: { id: true },
        });
        if (match) {
        ...
        if (await upsertJobPostingContact(job, { jobId: created.id, companyId: company.id, sourceLabel: provider.name, now: ts, companyName: job.companyNameRaw, isPlaceholderCompany: company.matchedBy === "placeholder" })) contactsCreated++;
      } catch (error) {
        report.failed++;
        const message = error instanceof Error ? error.message : String(error);
        report.errors.push(`${raw.externalId}: ${message}`);
        log.error("Offre non enregistrée", error, { provider: provider.key, externalId: raw.externalId });
      }
    }
    ...
    report.durationMs = now().getTime() - startedAt.getTime();
    await finishRun(run.id, { status: "ERROR", startedAt, now: now(), errors: report.errors, counters: { fetchedCount: report.fetched, createdCount: report.created, updatedCount: report.updated, duplicateCount: report.duplicates, rejectedCount: report.rejected, failedCount: report.failed } });
    await prisma.jobSource.update({ where: { id: source.id }, data: { lastSyncStatus: "ERROR", lastSyncError: message } });
    log.error("Ingestion échouée", error, { provider: provider.key, operation: "ingest", status: "error", errorCode: (error as { code?: string })?.code });
  }
  return report;
}
