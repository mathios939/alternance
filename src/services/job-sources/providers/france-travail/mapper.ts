...
import { departmentCodeFromPostal, findDepartmentByCode, isRegionName } from "@/config/departments";
...
export const ftOfferSchema = z.looseObject({
  ...
  lieuTravail: z
    .looseObject({ libelle: str, latitude: num, longitude: num, codePostal: str, commune: str })
    .nullish(),
  romeCode: str,
  romeLibelle: str,
  ...
  salaire: z
    .looseObject({ libelle: str, commentaire: str, complement1: str, complement2: str })
    .nullish(),
  ...
  origineOffre: z
    .looseObject({
      origine: z.union([z.string(), z.number()]).nullish(),
      urlOrigine: str,
      partenaires: z.array(z.looseObject({ nom: str, url: str, logo: str })).nullish(),
    })
    .nullish(),
});
...
export const FT_CANDIDATE_URL = (id: string) =>
...
export function cleanCityLabel(label: string | null | undefined): string | null {
  ...
  return stripped
    ...
    .split(/(\s+|-|')/)
    .map((part) =>
      /^[\s\-']$/.test(part) || part === "" ? part : part.charAt(0).toUpperCase() + part.slice(1),
    )
    .join("")
...
export function parseEducationRange(formations: FtOffer["formations"]): {
  min: EducationLevel | null;
  max: EducationLevel | null;
} {
  const levels = (formations ?? [])
    .map((f) => parseEducationLevel(f.niveauLibelle))
...
export function parseSalary(label: string | null | undefined): ParsedSalary {
  ...
  const t = label.toLowerCase();
  const period: SalaryPeriod | null = t.startsWith("mensuel")
    ? "MONTH"
    : t.startsWith("annuel")
      ? "YEAR"
      : t.startsWith("horaire")
        ? "HOUR"
        : null;
  if (!period) return { min: null, max: null, period: null };
  const amounts = [...t.matchAll(/(\d[\d\s]*(?:[.,]\d+)?)\s*(?:€|euros?)/g)]
...
export function parseContractType(
  natureContrat: string | null | undefined,
  alternance: boolean | null | undefined,
): RawJob["contractType"] {
...
export function mapFtOffer(o: FtOffer): RawJob {
  ...
  // « 44 - NANTES » : préfixe = code département. Mais « 52 - Pays de la Loire » ou « 44 - Grand Est » :
  // préfixe = code RÉGION (nomenclature distincte) → jamais interprété comme un département.
  const labelRest = lieu?.libelle?.replace(/^\s*(\d{2,3}|2[AB])\s*-\s*/i, "").trim() ?? "";
  const regionLevel =
    Boolean(lieu?.libelle) && !postalCode && !inseeCode && isRegionName(labelRest);
  const depCode = regionLevel
    ? null
    : (lieu?.libelle?.match(/^\s*(\d{2,3}|2[AB])\s*-/i)?.[1] ??
      departmentCodeFromPostal(postalCode) ??
      departmentCodeFromPostal(inseeCode));
  ...
  const applicationUrl =
    o.contact?.urlPostulation?.trim() ||
    (isPartner ? o.origineOffre?.urlOrigine?.trim() : null) ||
    o.contact?.urlRecruteur?.trim() ||
    sourceUrl;
  ...
  return {
    ...
    requirements: (o.qualitesProfessionnelles ?? [])
      .map((q) => q.libelle?.trim() ?? "")
      .filter(Boolean),
    city: regionLevel ? labelRest : cleanCityLabel(lieu?.libelle),
    postalCode,
    inseeCode,
    department: dep?.name ?? null,
    region: regionLevel ? labelRest : (dep?.region ?? null),
