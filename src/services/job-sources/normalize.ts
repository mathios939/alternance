 ...
 * Aucun champ n'est inventé : les valeurs manquantes restent nulles.
 */
export function normalizeJob(raw: RawJob, source: { key: string; type: NormalizedJob["source"]; isDemo: boolean }): NormalizedJob {
  ...
  return {
    ...
    slugBase: slugify(`${raw.title} ${raw.companyName} ${raw.city}`),
    ...
    city: city?.name ?? raw.city.trim(),
    postalCode: raw.postalCode ?? city?.postalCode ?? null,
    department: raw.department ?? city?.department ?? null,
    region: raw.region ?? city?.region ?? null,
    ...
    applicationUrl: raw.applicationUrl ?? raw.sourceUrl ?? null,
    sector,
    jobFamily,
