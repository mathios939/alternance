...
Les sources supplémentaires (API officielle La bonne alternance, flux carrières fournis, ATS à endpoint public, partenaires, dépôt direct) sont listées avec leur statut et ce qu'elles exigent dans `docs/PROVIDERS.md` ; aucune n'est activée sans validation réelle de bout en bout, et jamais par scraping interdit.
